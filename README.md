...
| 🧩 **Mod 管理** | 查看/启用/禁用/删除已装 Mod，可视化编辑配置项（说明文字常驻显示），一键把客户端 Mod 同步到服务器。支持 "Configs Extended" 这类 Mod 的集合/数组/字典/文本输入配置项。**Mod 配置集**：把一批 Mod 的启用状态+配置项打包保存为命名预设，一键应用到任意存档（合并式应用，自动识别缺失 Mod/过期配置项/非法取值并提示）。 |
