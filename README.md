...
存档浏览、Mod 配置与同步、世界/服务器配置可视化编辑。CLI 与 Tkinter GUI
双形态，也可以打包成单文件 `DSTCamp.exe` 分发给不装 Python 的用户。
...
- **本地服务器**：一键启动/停止本地专用服务器，每个已启动的世界有自己的
  ...
  份新的服务器存档。启动前会自动检查 `cluster_token.txt` 是否已配置，没
  配置的话会提醒（离线模式除外）。
...
```bash
python tests/test_e2e.py          # 核心模块（25 项）：Lua/INI 解析、存档发现、
