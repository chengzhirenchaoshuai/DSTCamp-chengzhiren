...
- Windows Defender 排除项功能全面修复与加强：检测覆盖扩展到三个目标、结果未知自动转管理员检测，并修复通配符崩溃、输出乱码、CLIXML 糊界面、路径框显示空白、修改误报失败等一系列问题。
- 统一主窗口与创建存档子窗口的 Mod 加载提示文案与字号，与"配置"按钮、创意工坊链接视觉风格一致。
- Mod 管理页签新增点击复制名称功能，主窗口与创建存档子窗口同步支持。
- Lua 解析器兼容 TheSim:SetPersistentString 写文件携带的 KLEI 头；世界设置读取 leveldataoverride.lua 失败时展示具体异常信息，便于远程排障。
- 修复高 DPI 缩放下界面比例失衡的问题，统一按显示器 DPI 换算像素字面量。
- 设置菜单调整：Windows 安全中心排除项移到语言下方，缓存目录设置改名，文件菜单新增清理缓存。
- 重写自动更新清理逻辑，固定安装文件名并自愈清理更新残留；清理存量 ZIP 版用户更新后遗留的僵尸 tools 目录。
