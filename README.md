...
**DSTCamp 是一款面向 Windows 的 Don't Starve Together 本地专用服务器管理工具。**

它把开服、配置、Mod、世界设置、备份和联机入口集中到一个清晰的图形界面中：支持 **Steam 与 WeGame 存档**，支持创建服务器存档，也支持单文件 EXE 或“EXE + tools/”ZIP 两种发布形态，解压即可使用。
...
| 页签 | 说明 |
|---|---|
| 🖥️ **本地服务器** | 一键启动/停止，每个世界独立控制台，支持公告、玩家列表、回档和复制为服务器存档。启动前校验令牌与端口；世界启动完成后核对 Mod 实际加载状态，并持续监控端口、运行库、权限、世界生成和 Lua/Mod 异常。 |
...
| 📦 **存档信息** | 存档详情 + 每个玩家角色状态（角色名/头像/血量/理智/饥饿/体温）；支持自动/手动备份、从备份恢复和备份策略配置。独立的「创建服务器存档」向导提供服务器配置、Mod 管理和世界设置。 |
...
- 将 Mod 加载状态与服务器异常诊断分层：正常加载显示成功横幅，单个 Mod 被禁用时只显示 Mod 状态提示。
