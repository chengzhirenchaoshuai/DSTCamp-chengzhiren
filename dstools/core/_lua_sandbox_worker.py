"""Child-process worker for lua_sandbox.run_lua_snippet.

Reads a Lua snippet from stdin, runs it under a Lua 5.1 interpreter
(matching Don't Starve Together's own engine version -- see lua_sandbox.py)
...
lua_sandbox.run_lua_snippet(), which owns the timeout/kill side of the
...
# The parent (lua_sandbox.run_lua_snippet) explicitly encodes/decodes the
...
def run_worker_main() -> None:
    ...
    try:
        ...
        # The parent (lua_sandbox.run_lua_snippet) never reads stderr --
