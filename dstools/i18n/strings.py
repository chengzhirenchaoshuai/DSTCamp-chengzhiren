...
STRINGS = {
    "zh": {
        ...
        "local.luajit_confirm_install_msg": "第三方开源项目 DontStarveLuaJIT2 提供的 LuaJIT 性能补丁。会复制一份 bin64 目录，装入创意工坊订阅内容里的注入文件和配套 Mod，真实安装不受影响。游戏或补丁更新后会自动提示重新生成副本。「卸载」只是关闭，不会删除已复制的副本。确定要继续吗？",
        ...
        "local.remove_junction_confirm_msg": "确定要删除服务器 mods/ 目录指向客户端 mods/ 文件夹的软链接吗？\n\n删除后专用服务器将无法加载客户端手动安装的 Mod（Steam 创意工坊订阅的 Mod 不受影响），直到重新建立软链接。",
    ...
    "en": {
        ...
        "local.remove_junction_confirm_msg": "Remove the junction that points the server's mods/ folder at the client's mods/ folder?\n\nAfter removal, the dedicated server won't be able to load manually installed client mods (Steam Workshop mods are unaffected) until the junction is recreated.",
