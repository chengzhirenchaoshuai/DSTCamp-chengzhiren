...
STRINGS = {
    "zh": {
        ...
        "selfhost.lobby_diag_result_title": "—— 诊断结论 ——",
        "selfhost.lobby_diag_result_frp": "✓ 已确认通过 VPS：游戏数据走 FRP。",
        "selfhost.lobby_diag_result_wireguard": "✓ 已确认通过 VPS：游戏 UDP 走 WireGuard/P2P。",
        "selfhost.lobby_diag_result_signal_only": "△ 部分生效：只确认 WireGuard 信令，尚未确认游戏数据经过 VPS。",
        "selfhost.lobby_diag_result_bypass": "△ 检测到 P2P 连接，但 FRP 或 WireGuard 游戏流量未达到确认阈值；可能是直连、中继或采样不足。",
        "selfhost.lobby_diag_result_inconclusive": "? 证据不足，无法确认本次游戏连接路线。请使用外网设备重新测试。",
        "selfhost.lobby_diag_evidence_connection": "专服证据：已认证={authenticated}，回环连接={loopback}，外部端口={ports}",
        "selfhost.lobby_diag_evidence_frp": "FRP 端口：{packets} 包 / {bytes} 字节",
        "selfhost.lobby_diag_evidence_wg": "WireGuard：接收增量 {rx} / 发送增量 {tx} 字节；隧道内非 STUN {inner} 字节；Mihomo 非 STUN {api} 字节",
    ...
    "en": {
        ...
        "selfhost.lobby_diag_result_frp": "✓ Confirmed through the VPS: game data used FRP.",
        "selfhost.lobby_diag_result_wireguard": "✓ Confirmed through the VPS: game UDP used WireGuard/P2P.",
        "selfhost.lobby_diag_result_signal_only": "△ Partially active: only WireGuard signaling was confirmed; game traffic through the VPS was not.",
        "selfhost.lobby_diag_result_bypass": "△ A P2P connection was detected, but FRP or WireGuard game traffic did not reach the confirmation threshold; this may be direct, relayed, or an insufficient sample.",
        "selfhost.lobby_diag_result_inconclusive": "? Insufficient evidence to identify the route. Test again with a device on another network.",
        "selfhost.lobby_diag_evidence_connection": "Server evidence: authenticated={authenticated}, loopback={loopback}, external ports={ports}",
        "selfhost.lobby_diag_evidence_frp": "FRP port: {packets} packets / {bytes} bytes",
        "selfhost.lobby_diag_evidence_wg": "WireGuard: RX delta {rx} / TX delta {tx} bytes; inner non-STUN {inner} bytes; Mihomo non-STUN {api} bytes",
