...
DSTCamp（包名 `dstools`，当前版本 `1.3.7`）是 Windows 上的《饥荒：联机版》本地服务器管理工具，GUI 入口为 `dst-gui`。支持 Steam/WeGame 存档、Mod、世界设置、服务器配置、本地专服、内网穿透和大厅加速。

1.3.7 重点扩展「深埋之下」与「永不妥协」Mod 兼容、适配 LuaJIT 补丁新版注入，并延续世界设置页视口虚拟化等内存优化。
...
- 世界设置视口虚拟化必须保持筛选、排序、滚动定位与选中状态一致；「深埋之下 / 永不妥协」兼容按实际 Mod 源码核对 key 与值域。
- LuaJIT 补丁新版注入布局、自动更新命名与进度显示需与远程版本检测和冒烟测试保持一致。
